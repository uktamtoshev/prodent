...
const errorMessage = (error: MutationError) => error.message || 'Error';
...
export function DoctorProfileHeader({
  ...
  const handleShare = async () => {
    try {
      ...
      toast({
        title: t('doctorProfileHeader.linkCopied'),
        description: t('doctorProfileHeader.linkCopied'),
      });
    } catch {
      toast({
        title: t('doctorInlineEdit.error'),
        description: t('doctorInlineEdit.error'),
        variant: 'destructive',
      });
    }
  };
  ...
  const updateName = useMutation({
    ...
    onError: (error: MutationError) => {
      toast({ title: t('doctorEditProfile.saveError'), description: errorMessage(error), variant: 'destructive' });
    },
  });
  ...
  const uploadCover = async (file: File) => {
    ...
    try {
      ...
      toast({ title: t('doctorEditProfile.saveError'), description: error instanceof Error ? error.message : 'Error', variant: 'destructive' });
    } finally {
  ...
  const uploadAvatar = async (file: File) => {
    ...
    try {
      ...
      toast({ title: t('doctorEditProfile.avatarError'), description: error instanceof Error ? error.message : 'Error', variant: 'destructive' });
    } finally {
      setUploadingAvatar(false);
    }
  };

  // Default cover — clinical dental-chair scene shipped in /public so the
  // header isn't an empty grey rectangle for doctors who haven't uploaded
  // their own banner yet.
        ...
        <div className="bg-gradient-to-r from-amber-500 via-amber-400 to-amber-500 text-white py-2 px-4 flex items-center justify-center gap-2 text-sm font-medium">
          {hasProfileFrame && !isGoldPlan ? (
            <>
              ...
              <span>{t('doctorProfileHeader.premiumBadge')}</span>
              <Sparkles className="w-4 h-4" />
            </>
          ) : (
            <>
        ...
        {isOwner && (
          <>
            ...
            <Button
              ...
              aria-label={uploadingCover ? t('crmFileComponents.uploading') : t('doctorProfileHeader.editProfile')}
            >
              <Camera className="w-4 h-4" />
              {uploadingCover ? t('crmFileComponents.uploading') : t('doctorProfileHeader.editProfile')}
            </Button>
          </>
        )}
      </div>
        ...
        <div className="relative pb-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end gap-4">
            <div className="relative -mt-[84px] sm:-mt-[96px] self-center sm:self-auto z-10">
              ...
              {showPremiumFrame && (
                <div className="absolute -top-2 -right-2 bg-gradient-to-br from-amber-400 to-amber-600 rounded-full p-2 shadow-lg border-2 border-card">
              ...
              {isOwner && (
                <>
                  ...
                  <Button
                    variant="secondary"
                    size="icon"
                    className="absolute bottom-3 right-3 h-11 w-11 rounded-full bg-primary text-primary-foreground shadow-lg hover:bg-primary/90 focus-visible:ring-2 focus-visible:ring-ring"
                    onClick={() => avatarInputRef.current?.click()}
                    disabled={uploadingAvatar}
                    aria-label={uploadingAvatar ? t('crmFileComponents.uploading') : t('doctorEditProfile.avatarPhoto')}
                  >
                    {uploadingAvatar ? (
            ...
            <div className="flex-1 text-center sm:text-left sm:pb-4 sm:pt-2">
              ...
              {isOwner && isEditingName ? (
                <div className="flex min-w-0 flex-wrap items-center justify-center sm:justify-start gap-2 mb-1">
                  <Label htmlFor="doctor-profile-name" className="sr-only">
                  ...
                  <Input
                    ...
                    className="h-11 w-full min-w-0 text-2xl font-bold px-2 sm:w-auto sm:max-w-[280px] border-primary"
                    autoFocus
                  />
                  <Button
                    size="icon"
                    ...
                    aria-label={t('doctorEditProfile.save')}
                  >
                  ...
                  <Button
                    ...
                    variant="ghost"
                    className="h-11 w-11 focus-visible:ring-2 focus-visible:ring-ring"
                    onClick={() => { setIsEditingName(false); setEditedName(doctor.profile?.full_name || ''); }}
                    aria-label={t('doctorEditProfile.cancel')}
                  >
                ...
                <div className="flex items-center justify-center sm:justify-start gap-2 group mb-1">
                  <h1 className="min-w-0 break-words text-2xl sm:text-3xl font-bold text-foreground leading-tight">
                    {doctor.profile?.full_name || t('crmDoctorMgmt.roleDoctor')}
                  </h1>
                  {isOwner && (
                    <Button 
                      size="icon" 
                      variant="ghost" 
                      className="h-11 w-11 shrink-0 opacity-0 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100 focus-visible:opacity-100 focus-visible:ring-2 focus-visible:ring-ring"
                      onClick={() => setIsEditingName(true)}
                      aria-label={t('doctorProfileHeader.editProfile')}
                    >
              ...
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-1 text-base text-muted-foreground">
                ...
                <span>{t('doctorProfileHeader.followBtn')}</span>
                ...
                <span>{doctor.experience_years} {t('doctorProfileHeader.yearsExp')}</span>
              </div>
            </div>
                ...
                <>
                  {!isViewerDoctor && (
                    <Button 
                      variant="secondary" 
                      onClick={onMessage} 
                      className="min-h-11 min-w-0 flex-1 gap-2 font-semibold focus-visible:ring-2 focus-visible:ring-ring sm:flex-none"
                    >
                      <MessageCircle className="w-4 h-4" />
                      {t('doctorProfileHeader.sendMessage')}
                    </Button>
                  )}
                  <Button
                  ...
                  >
                    ...
                    {isFollowing ? t('doctorProfileHeader.unfollowBtn') : t('doctorProfileHeader.followBtn')}
                  </Button>
                </>
              )}
              <DropdownMenu>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-3 mt-3 border-t border-border text-base">
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-4">
              {doctor.clinic && (
                <>
                  <div className="flex items-center gap-1.5 text-muted-foreground">
                    <Building2 className="w-4 h-4" />
                    <span>{t('doctorClinicInvitations.currentClinics')}: <span className="text-foreground font-medium hover:underline cursor-pointer">{doctor.clinic.name}</span></span>
                  </div>
                  <div className="flex items-center gap-1.5 text-muted-foreground">
              ...
              <div className="flex items-center justify-center sm:justify-end">
                {showPhone ? (
                  <a 
                    ...
                    className="flex min-h-11 items-center gap-2 rounded-md px-2 text-primary font-medium hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                  >
                  ...
                  <Button
                    ...
                    className="min-h-11 gap-2 focus-visible:ring-2 focus-visible:ring-ring"
                  >
                    <Eye className="w-4 h-4" />
                    {t('doctorProfileHeader.callDoctor')}
                  </Button>
                )}
              </div>
            )}
