...
import { X, Calendar, User, FileText, Camera, Package, Lightbulb, History, ArrowRight } from "lucide-react";
...
export function ToothDetailCard({ tooth, isChild = false, onClose }: ToothDetailCardProps) {
  ...
  return (
    <Card className="w-full border-border/40 shadow-lg bg-card/95 backdrop-blur-sm animate-scale-in overflow-hidden">
      {/* Header with gradient accent */}
      <CardHeader className="pb-4 pt-5 px-5 relative bg-gradient-to-r from-muted/50 to-transparent">
        <Button
          variant="ghost"
          size="icon"
          ...
          <div className="space-y-5">
            ...
            {tooth.images && tooth.images.length > 0 && (
              <div className="space-y-3">
                <div className="flex items-center gap-2 text-sm font-medium">
                  <Camera className="h-4 w-4 text-primary" />
                  Фотографии ({tooth.images.length})
                </div>
                <div className="grid grid-cols-4 gap-2">
                  {tooth.images.slice(0, 4).map((img, idx) => (
                    <div 
                      key={idx} 
                      className="aspect-square rounded-lg bg-muted overflow-hidden border border-border/30"
                    >
                      <img 
                        src={img} 
                        alt={`Зуб ${tooth.tooth_number}`}
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  {tooth.procedures!.map((proc, idx) => (
                    <div 
                      key={idx}
                      className="p-3 rounded-xl bg-muted/50 border border-border/30 hover:bg-muted/70 transition-colors"
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <div className="text-right space-y-1">
                          <span className="text-xs text-muted-foreground block">{proc.date}</span>
                          {proc.price && (
            ...
            {/* Footer info */}
            {(tooth.doctor_name || tooth.updated_at) && (
              <>
                ...
                <div className="flex items-center justify-between text-xs text-muted-foreground">
                  {tooth.doctor_name && (
                    <div className="flex items-center gap-1.5">
                      ...
                      <span>Лечащий врач: <span className="font-medium text-foreground">{tooth.doctor_name}</span></span>
                    </div>
                  )}
                  {tooth.updated_at && (
                    <div className="flex items-center gap-1.5">
                      <Calendar className="h-3.5 w-3.5" />
                      {format(new Date(tooth.updated_at), 'd MMMM yyyy', { locale: ru })}
                    </div>
                  )}
                </div>
              </>
            ...
            {!hasBeforeAfter && !hasProcedures && !tooth.notes && !tooth.materials && !tooth.recommendations && tooth.status === 'healthy' && (
              <div className="text-center py-6 text-muted-foreground">
                ...
                <p className="text-sm font-medium">Зуб здоров</p>
                <p className="text-xs mt-1">Лечение не требуется</p>
              </div>
            )}
          </div>
        </ScrollArea>
